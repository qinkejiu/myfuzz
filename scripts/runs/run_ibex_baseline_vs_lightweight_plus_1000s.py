#!/usr/bin/env python3
"""Run Ibex baseline vs a stronger lightweight top-level harness for 1000s."""
...
SCHEMES = [
    ...
    },
    {
        "scheme": "scheme4p",
        "label": "scheme4_plus",
        "config": "configs/designs/ibex_lightweight_plus/config.json",
        "reuse_out_dir": "runs/designs/ibex_lightweight_plus",
        "description": "Top-level ibex_core lightweight harness with extra low-frequency exploration.",
    },
]
    ...
    for item in prepared:
        ...
        coverage_counts[item["scheme"]] = {
            ...
            "metadata_label_audit": {
                ...
                "note": "Coverage rows are counted by actual __vi_coverage bit index, not by TOML human labels.",
            },
        }
...
def main() -> int:
    ...
    parser.add_argument("--label", default="ibex_baseline_vs_lightweight_plus_1000s")
    ...
    parser.add_argument("--final-grace-seconds", type=int, default=300)
    args = parser.parse_args()
    sample_interval = int(args.sample_interval or args.seconds)
    if sample_interval <= 0:
        raise ValueError("--sample-interval must be positive")

    root = args.repo.resolve()
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_id = f"{args.label}_{args.seconds}s_{stamp}"
    ...
    manifest = {
        ...
        "coverage_metric": (
            ...
            f"common to both schemes, excluding the top module MSB slice of {args.exclude_top_bits} bits."
        ),
    ...
    try:
        ...
        hour_index = 1
        while True:
            while next_target < args.seconds and time.time() >= started + next_target:
                sample_all(run_root, runs, started, hour_index, next_target, indices)
                hour_index += 1
                next_target += sample_interval
            remaining = deadline - time.time()
            if remaining <= 0:
                break
            sleep_target = min(deadline, started + next_target)
            time.sleep(min(10, max(0.0, sleep_target - time.time()), remaining))
        sample_all(run_root, runs, started, hour_index, args.seconds, indices)
        ...
        manifest["schemes"] = [
            ...
            for item, run in zip(manifest["schemes"], runs)
        ]
        write_json(run_root / "manifest.json", manifest)
        return 0
    finally:
