...
fn main() {
	let matches = App::new(APP_NAME).version(VERSION).author(AUTHOR)
		...
		.arg(Arg::with_name("seed_cycles")
			...
			.takes_value(true)
			.default_value("5"))
		.arg(Arg::with_name("campaign_seed")
			.long("campaign-seed")
			.help("Seed all non-deterministic mutation strategies reproducibly.")
			.takes_value(true)
			.default_value("1"))
	...
	let args = Args {
		...
		campaign_seed: matches.value_of("campaign_seed").unwrap().parse::<u64>().unwrap(),
	};
