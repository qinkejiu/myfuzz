...
impl MutationSchedule {
	pub fn initialize(config: MutationScheduleConfig, test_size: TestSize, input: Vec<(String,u32)>) -> Self {
		...
		let low = config.campaign_seed as u32;
		let high = (config.campaign_seed >> 32) as u32;
		let rng_seed : Seed = [low ^ 0x243f6a88, high ^ 0x85a308d3,
		                         low ^ 0x13198a2e, high ^ 0x03707344];
		let rng = RefCell::new(rand::XorShiftRng::from_seed(rng_seed));
		MutationSchedule { config, format, input_size, mutators, mutator_id_to_name, rng }
	}
