...
- Timeout creates exactly one error completion. If the backend request was accepted, the arbiter quarantines and discards its late response before accepting another request.
- Reset aborts active requests without completion and restores normal arbitration after release.
...
- After an accepted backend request times out, the arbiter waits in quarantine for that request's late response before reusing the untagged single-outstanding backend. Reset remains the recovery path if a non-conforming backend never produces the late response.
- The regression run is green but not warning-free because of pre-existing `TemporaryDirectory` cleanup warnings in `tests/composition/test_processor_execution.py`.
