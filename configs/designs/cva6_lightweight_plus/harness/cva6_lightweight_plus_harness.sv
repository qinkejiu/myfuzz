// Long-run oriented lightweight constrained harness for CVA6.
//
// This keeps CVA6 as a top-level DUT and replaces fully-random environment
// inputs with stateful NoC/AXI, CV-X-IF, interrupt, and debug behavior.

module cva6_lightweight_plus_harness (
    input  logic          clock,
    input  logic          reset,
    ...
    always_ff @(posedge clock or negedge rst_ni) begin
        if (!rst_ni) begin
            boot_addr_q <= base_seed[3] ? {boot_seed[63:3], 3'b000} : 64'h0000_0000_8000_0000;
            hart_id_q <= hart_seed;
        end
    end
    ...
    wire aw_ready_w = !wr_pending_q && !b_valid_q &&
                      (base_seed[0] || cycle_q[0] || cycle_q[5]);
    wire w_ready_w = !wr_pending_q && !b_valid_q &&
                     (base_seed[1] || cycle_q[1] || cycle_q[6]);
    wire ar_ready_w = !rd_pending_q && !r_valid_q &&
                      (base_seed[2] || cycle_q[0] || cycle_q[4]);
        ...
        if (!rst_ni) begin
            ...
            if (!wr_pending_q && !b_valid_q) begin
                ...
                if (wr_delay_q == 3'h0) begin
                    ...
                    b_valid_q <= 1'b1;
                    b_id_q <= wr_id_q;
                    b_resp_q <= (base_seed[18] && (cycle_q[8:0] == {1'b0, event_seed[7:0]})) ? 2'b10 : 2'b00;
                end else begin
            ...
            if (!rd_pending_q && !r_valid_q && ar_accept) begin
                ...
                rd_len_q <= (noc_ar_len > 8'd7) ? 8'd7 : noc_ar_len;
                rd_beat_q <= 8'h0;
                ...
                if (rd_delay_q == 3'h0) begin
                    ...
                    r_resp_q <= (base_seed[31] && (cycle_q[9:0] == {2'b01, event_seed[15:8]})) ? 2'b10 : 2'b00;
    ...
    always_ff @(posedge clock or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            event_period_q <= 8'd96 + event_seed[5:0];
            event_duration_q <= {5'h0, event_seed[10:8]} + 8'h2;
            event_select_q <= event_seed[14:11];
            event_active_q <= 1'b0;
        end else begin
            if (event_counter_q >= event_period_q) begin
                event_counter_q <= 8'h0;
                event_period_q <= 8'd96 + (event_seed[23:16] ^ base_seed[7:0]);
                event_duration_q <= {5'h0, event_seed[18:16]} + 8'h2;
                event_select_q <= event_seed[30:27] ^ cycle_q[7:4];
                event_active_q <= base_seed[43] || event_seed[31];
            end else begin
