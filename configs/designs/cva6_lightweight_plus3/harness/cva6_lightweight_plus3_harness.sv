// Long-run oriented lightweight-plus3 constrained harness for CVA6.
//
// This keeps CVA6 as a top-level DUT and replaces fully-random environment
// inputs with bit-controlled mixed raw/structured NoC modes, an input-bit
// projected instruction stream, CV-X-IF, interrupt, debug, and low-rate error
// behavior.

module cva6_lightweight_plus3_harness (
    input  logic          clock,
    input  logic          reset,
    ...
    wire        raw_noc_mode_w = extra_seed[8] ^ base_seed[46];
    wire        random_read_mode_w = extra_seed[9] ^ base_seed[47];
    wire        fast_response_mode_w = extra_seed[10] ^ base_seed[48];
    wire        trap_storm_mode_w = extra_seed[11] ^ base_seed[49];
    ...
    always_ff @(posedge clock or negedge rst_ni) begin
        if (!rst_ni) begin
            boot_addr_q <= base_seed[3]
                         ...
                         : (64'h0000_0000_8000_0000 + {54'h0, scenario_w, 3'b000});
    ...
    wire aw_ready_w = raw_noc_mode_w ? rfuzz_input_bits[500] :
                      (!wr_pending_q && !b_valid_q &&
                       (fast_response_mode_w || base_seed[0] || cycle_q[0] || cycle_q[5] || (scenario_w[0] && !cycle_q[3])));
    wire w_ready_w = raw_noc_mode_w ? rfuzz_input_bits[501] :
                     (!wr_pending_q && !b_valid_q &&
                      (fast_response_mode_w || base_seed[1] || cycle_q[1] || cycle_q[6] || (scenario_w[1] && !cycle_q[4])));
    wire ar_ready_w = raw_noc_mode_w ? rfuzz_input_bits[502] :
                      (!rd_pending_q && !r_valid_q &&
                       (fast_response_mode_w || base_seed[2] || cycle_q[0] || cycle_q[4] || (scenario_w[2] && !cycle_q[5])));
        ...
        if (!rst_ni) begin
            ...
            if (!rd_pending_q && !r_valid_q && ar_accept) begin
                ...
                if (rd_delay_q == 3'h0) begin
                    ...
                    r_valid_q <= 1'b1;
                    r_id_q <= rd_id_q;
                    if (!base_seed[55] || (!random_read_mode_w && (rd_addr_q[31] || (rd_addr_q[15:12] == boot_addr_q[15:12])))) begin
                        r_data_q <= instr_pair(rd_addr_q, rd_beat_q, scenario_w, mem_seed0[31:0] ^ mem_seed1[63:32]);
                    end else if (raw_noc_mode_w || random_read_mode_w) begin
                        r_data_q <= rfuzz_input_bits[255:192] ^ mix64(mem_seed1, rd_addr_q, {56'h0, rd_beat_q});
                    end else begin
    ...
    always_ff @(posedge clock or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            event_period_q <= trap_storm_mode_w ? (8'd8 + {5'h0, event_seed[2:0]}) :
                                                   (8'd24 + {3'b000, event_seed[4:0]});
            event_duration_q <= trap_storm_mode_w ? (8'd1 + {5'h0, event_seed[10:8]}) :
                                                    ({5'h0, event_seed[10:8]} + 8'h1);
            event_select_q <= event_seed[14:11];
            event_active_q <= 1'b0;
        end else begin
            if (event_counter_q >= event_period_q) begin
                event_counter_q <= 8'h0;
                event_period_q <= trap_storm_mode_w ? (8'd8 + {5'h0, (event_seed[18:16] ^ scenario_w[2:0])}) :
                                                       (8'd24 + {3'b000, (event_seed[20:16] ^ base_seed[4:0])});
                event_duration_q <= {5'h0, (event_seed[18:16] ^ scenario_w[2:0])} + (trap_storm_mode_w ? 8'h1 : 8'h2);
                event_select_q <= event_seed[30:27] ^ cycle_q[6:3] ^ {1'b0, scenario_w[2:0]};
                event_active_q <= base_seed[43] || event_seed[31] || trap_storm_mode_w || (cycle_q[5:0] == event_seed[5:0]);
            end else begin
    ...
    wire [1:0] irq_i_w = event_active_q ? (event_select_q[1:0] ^ scenario_w[1:0]) : 2'b00;
    wire ipi_i_w = event_active_q && ((event_select_q == 4'h2) || (trap_storm_mode_w && cycle_q[3]));
    wire time_irq_i_w = event_active_q && ((event_select_q == 4'h3) || (trap_storm_mode_w && cycle_q[4]));
    wire debug_req_i_w = (base_seed[44] && event_active_q && event_select_q == 4'h4) ||
                         ...
                         (trap_storm_mode_w && event_active_q && event_select_q[2:0] == 3'h5);
