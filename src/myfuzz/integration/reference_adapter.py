...
_PIPE_DRAIN_POLL_SECONDS = 0.025
_PIPE_DRAIN_JOIN_SECONDS = 0.25
...
class _BoundedPipeDrain:
    ...
    def __init__(self, stream: object) -> None:
        ...
        try:
            self._descriptor: int | None = self._stream.fileno()  # type: ignore[attr-defined]
            ...
            while True:
                ...
                if self._descriptor is None:
                    ...
                    readable, _writable, _exceptional = select.select(
                        (self._descriptor,), (), (), _PIPE_DRAIN_POLL_SECONDS
                    )
                    if not readable:
                        continue
                    try:
    ...
    def finish(self) -> str:
        self._stop.set()
        self._thread.join(_PIPE_DRAIN_JOIN_SECONDS)
        stream_closed = False
        try:
            if self._thread.is_alive():
                ...
                self._thread.join(_PIPE_DRAIN_JOIN_SECONDS)
            if self._thread.is_alive():
                raise RuntimeError("reference evaluator stderr drain could not stop")
...
def _run_cleanup(action: object, original: BaseException | None) -> None:
    try:
        action()  # type: ignore[operator]
    except BaseException as cleanup_error:
        if original is None:
            raise
        _report_cleanup_failure(original, cleanup_error)
            ...
            try:
                ...
                try:
                    ...
                    if status_writer is not None:
                        _run_cleanup(lambda: os.close(status_writer), original_error)
                    status_writer = None
                ...
                if process is not None and not supervisor_finished:
                    try:
                        ...
                        if original_error is None:
                            raise
                        _report_cleanup_failure(original_error, cleanup_error)
                if status_writer is not None:
                    _run_cleanup(lambda: os.close(status_writer), original_error)
                if status_reader is not None:
                    _run_cleanup(lambda: os.close(status_reader), original_error)
                if stderr_drain is not None:
                    drain = stderr_drain
                    stderr_drain = None
                    _run_cleanup(drain.finish, original_error)
