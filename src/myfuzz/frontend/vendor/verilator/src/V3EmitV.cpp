...
class EmitVBaseVisitorConst VL_NOT_FINAL : public VNVisitorConst {
    ...
    void visit(AstNodeModule* nodep) override {
        putfs(nodep, nodep->verilogKwd() + " " + nodep->origName());
        std::vector<std::pair<int, string>> ports;
        for (AstNode* stmtp = nodep->stmtsp(); stmtp; stmtp = stmtp->nextp()) {
            if (const AstPort* const portp = VN_CAST(stmtp, Port)) {
                ports.emplace_back(portp->pinNum(), portp->name());
            }
        }
        if (ports.empty()) {
            for (AstNode* stmtp = nodep->stmtsp(); stmtp; stmtp = stmtp->nextp()) {
                const AstVar* const varp = VN_CAST(stmtp, Var);
                if (varp && varp->isIO() && varp->pinNum() > 0) {
                    ports.emplace_back(varp->pinNum(), varp->origName());
                }
            }
        }
    ...
    void visit(AstCell* nodep) override {
        const AstNodeModule* const modp = nodep->modp();
        putfs(nodep, modp ? modp->origName() : nodep->modName());
        if (nodep->paramsp()) {
            puts(" #(");
        ...
        for (AstPin* pinp = nodep->pinsp(); pinp; pinp = VN_AS(pinp->nextp(), Pin)) {
            puts(".");
            puts(pinp->modVarp() ? pinp->modVarp()->origName() : pinp->name());
            puts("(");
            iterateConstNull(pinp->exprp());
            puts(pinp->nextp() ? "), " : ")");
        }
