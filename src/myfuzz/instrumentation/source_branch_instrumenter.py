    ...
    for tok in iter_tokens(masked, module.header_end, module.end):
        if tok.text in runtime_words:
            ...
            if is_begin_at(masked, body_pos):
                ...
                range_end = None if end is None else end + len("end")
            elif word_at(masked, body_pos, "if"):
                range_end = if_statement_end(masked, body_pos, module.end)
            else:
                end = find_statement_end(masked, body_pos, module.end)
                range_end = None if end is None else end + 1
            if range_end is not None:
                ranges.append(RuntimeRange(tok.start, range_end, tok.text, body_pos))
...
def skip_sv_attributes(masked: str, pos: int, limit: int) -> int:
    pos = skip_ws(masked, pos, limit)
    while pos + 1 < limit and masked[pos : pos + 2] == "(*":
        end = masked.find("*)", pos + 2, limit)
        if end < 0:
            return pos
        pos = skip_ws(masked, end + 2, limit)
    return pos
...
def statement_body_supported(masked: str, body_pos: int, limit: int) -> bool:
    tok = next_token(masked, skip_sv_attributes(masked, body_pos, limit), limit)
    if tok is None:
        return False
...
def instrument_single_statement_body(
    ...
    if stmt_end is None:
        return None
    # Do not wrap statements that are split by preprocessor conditionals.
    # Adding begin/end across `ifdef/`elsif/`else/`endif can move the
    # generated block boundaries into only one macro branch and break syntax.
    if re.search(r"(?m)^[ \t]*`(?:ifn?def|elsif|else|endif)\b", text[body_pos : stmt_end + 1]):
        return None
...
def instrument_branch_body(
    ...
    attr_body_pos = skip_sv_attributes(masked, body_pos, limit)
    if is_begin_at(masked, attr_body_pos):
        insertion = instrument_begin_body(text, masked, attr_body_pos, limit, signal, comment, order)
        if insertion is None:
            return None
        end = find_matching_begin(masked, attr_body_pos, limit)
        if end is None:
            return None
...
def make_instance_connection_insertions(
    ...
    child_counter = 0
    for inst in sorted(plan.instances, key=lambda item: (item.name, item.child, item.line, item.column)):
        child_width = widths.get(inst.child, 0)
...
def make_hierarchy_body_insertion(
    plan: ModulePlan,
    coverage_port: str,
    coverage_epoch_port: str | None = None,
) -> Insertion | None:
    ...
    terms: list[str] = [
        (f"({point.signal}__epoch == {coverage_epoch_port})" if coverage_epoch_port else point.signal)
...
def add_hierarchy_insertions(
    ...
    *,
...
) -> tuple[dict[str, ModulePlan], list[str], set[str], list[dict[str, object]], dict[str, int]]:
    ...
    for plan in module_map.values():
        ...
        if coverage_epoch_port:
            plan.insertions.extend(make_epoch_port_insertions(
                plan, coverage_epoch_port, coverage_epoch_width, skipped,
            ))
            epoch_declarations = [
                f"logic [{coverage_epoch_width - 1}:0] {point.signal}__epoch = '0;"
                for point in plan.points if not point.expr
            ]
            if epoch_declarations:
                indent = line_indent(plan.text, plan.region.header_end)
                text = "\n" + "\n".join(f"{indent}{line}" for line in epoch_declarations) + "\n"
                plan.insertions.append(Insertion(plan.region.header_end, text, 0))
            for index, insertion in enumerate(plan.insertions):
                updated = insertion.text
                for point in plan.points:
                    updated = updated.replace(
                        f"{point.signal} = 1'b1;",
                        f"{point.signal}__epoch = {coverage_epoch_port}; {point.signal} = 1'b1;",
                    )
                if updated != insertion.text:
                    plan.insertions[index] = Insertion(insertion.offset, updated, insertion.order)
    for plan in module_map.values():
        if plan.region.name not in active_modules or widths.get(plan.region.name, 0) <= 0:
            continue
        make_instance_connection_insertions(plan, coverage_port, widths, active_modules)
        if coverage_epoch_port:
            make_epoch_instance_connection_insertions(
                plan, coverage_epoch_port, active_modules, widths,
            )
        body_insertion = make_hierarchy_body_insertion(plan, coverage_port, coverage_epoch_port)
    ...
    return module_map, roots, active_modules, module_summary, skipped


def make_epoch_port_insertions(
    plan: ModulePlan, epoch_port: str, epoch_width: int, skipped: dict[str, int],
) -> list[Insertion]:
    port_list = find_module_port_list(plan.masked, plan.region)
    if port_list is None:
        skipped["module_without_port_list_for_epoch"] = skipped.get("module_without_port_list_for_epoch", 0) + 1
        return []
    port_open, port_close = port_list
    if header_uses_ansi_ports(plan.masked, port_open, port_close):
        item = f"input wire [{epoch_width - 1}:0] {epoch_port}"
        return [Insertion(append_list_item_offset(plan.masked, port_open, port_close),
                          append_list_item_text(plan.text, plan.masked, port_open, port_close, item), -4)]
    indent = line_indent(plan.text, plan.region.header_end)
    return [
        Insertion(append_list_item_offset(plan.masked, port_open, port_close),
                  append_list_item_text(plan.text, plan.masked, port_open, port_close, epoch_port), -4),
        Insertion(plan.region.header_end, f"\n{indent}input wire [{epoch_width - 1}:0] {epoch_port};\n", -2),
    ]


def make_epoch_instance_connection_insertions(
    plan: ModulePlan, epoch_port: str, active_modules: set[str], widths: dict[str, int],
) -> None:
    for index, inst in enumerate(sorted(plan.instances, key=lambda item: (item.name, item.child))):
        if inst.child not in active_modules or widths.get(inst.child, 0) <= 0:
            continue
        item = f".{epoch_port}({epoch_port})" if inst.named_ports else epoch_port
...
def _source_line(text: str, line: int) -> str:
    ...
    return " ".join(lines[line - 1].strip().split()) if 0 < line <= len(lines) else ""


def _definition_points(plan: ModulePlan) -> list[dict[str, object]]:
    source_id = hashlib.sha256(plan.text.encode()).hexdigest()
    runtime_ranges = collect_runtime_ranges(plan.masked, plan.region)
    ordinals: dict[tuple[str, str, str], int] = {}
    result: list[dict[str, object]] = []
    for point in plan.points:
        lines = plan.text.splitlines(keepends=True)
        point_offset = sum(len(value) for value in lines[:max(point.line - 1, 0)]) + max(point.column - 1, 0)
        runtime = containing_range(point_offset, runtime_ranges)
        if runtime is None:
            process_kind = "expression" if point.kind == "expression" else "unknown_process"
        elif runtime.kind == "always":
            header = plan.masked[runtime.start:runtime.begin_pos]
            process_kind = "edge_always" if re.search(r"@\s*\([^)]*\b(?:pos|neg)edge\b", header) else "always"
        else:
            process_kind = runtime.kind
        identity_text = " ".join((point.expr or _source_line(plan.text, point.line)).split())
        key = (point.kind, point.subtype, identity_text)
        ordinal = ordinals.get(key, 0)
        ordinals[key] = ordinal + 1
        node_payload = {
            "module": plan.region.name,
            "source_id": source_id,
            "kind": point.kind,
        ...
        node_id = hashlib.sha256(_canonical_json(node_payload)).hexdigest()
        result.append({
            "source_id": source_id,
            "node_id": node_id,
            "module": plan.region.name,
            ...
            "subtype": point.subtype,
            "fail": point.fail,
            "source_file": plan.rel_path.as_posix(),
...
def build_coverage_abi(
    ...
    *,
    ...
    if len(roots) != 1:
        raise SystemExit("CoverageABI v1 requires exactly one top module")
    unknown_requirements = (required_modules | optional_modules) - active_modules
    if unknown_requirements:
        raise SystemExit(
            "Coverage requirement names module(s) outside the elaborated hierarchy: "
            + ", ".join(sorted(unknown_requirements))
        )
    top = roots[0]
    definitions = {name: _definition_points(plan) for name, plan in module_map.items()}
    included: list[dict[str, object]] = []
    excluded: list[dict[str, object]] = []
    visited_paths: set[str] = set()

    def visit(module: str, hierarchy: str, parent_exports: bool, ancestors: tuple[str, ...]) -> None:
        if module in ancestors:
            raise SystemExit(f"recursive module hierarchy at {hierarchy}")
        if hierarchy in visited_paths:
            raise SystemExit(f"recursive instance hierarchy at {hierarchy}")
        visited_paths.add(hierarchy)
        plan = module_map[module]
        exports = module_can_export_coverage(plan) and plan.coverage_width > 0
        transported = parent_exports and exports
        requirement = "required" if module in required_modules else "optional"
        if module in required_modules and not transported:
            raise SystemExit(f"required module {module} cannot export coverage at {hierarchy}")
        if transported:
            skip_reason = None
        elif not parent_exports:
            skip_reason = "ancestor_does_not_export_coverage"
        elif not module_can_export_coverage(plan):
            skip_reason = "module_has_no_port_list"
        else:
            skip_reason = "module_has_no_exportable_points"
        for definition in definitions[module]:
            point_payload = {
                "source_id": definition["source_id"],
                "node_id": definition["node_id"],
                "hierarchy": hierarchy,
                "coverage_type": f"{definition['kind']}.{definition['subtype']}",
            }
            entry = dict(definition)
            entry.update({
                "point_id": hashlib.sha256(_canonical_json(point_payload)).hexdigest(),
                "hierarchy": hierarchy,
                "included": transported,
                "offset": None,
                "requirement": requirement,
                "skip_reason": skip_reason,
            })
            (included if transported else excluded).append(entry)
        for inst in sorted(plan.instances, key=lambda item: (item.name, item.child, item.line, item.column)):
            if inst.child in active_modules and inst.child in module_map:
                visit(inst.child, f"{hierarchy}.{inst.name}", transported, ancestors + (module,))

    visit(top, top, True, ())
    width = len(included)
    for index, entry in enumerate(included):
        entry["offset"] = width - index - 1
    excluded.sort(key=lambda item: (str(item["hierarchy"]), str(item["point_id"])))
    catalog = included + excluded
    abi_payload = {
        "schema": "myfuzz.coverage-abi/v1",
        "port_name": coverage_port,
        "width": width,
        "reset_semantics": "process_initial_zero_sticky",
        "rfuzz_handshake": "coverage_valid_o_when_done_o",
        "points": catalog,
    }
    digest = hashlib.sha256(_canonical_json(abi_payload)).hexdigest()
    return {
        "schema": "myfuzz.coverage-abi/v1",
...
def _resolve_instrumentation_name(
    requested: str, files: Iterable[Path], project_root: Path,
) -> str:
    texts = [(rel_to_project(path, project_root).as_posix(), path.read_text(errors="ignore")) for path in files]
    if not any(re.search(rf"\b{re.escape(requested)}\b", text) for _, text in texts):
        return requested
    digest = hashlib.sha256(_canonical_json(texts)).hexdigest()[:12]
    candidate = f"{requested}__{digest}"
    suffix = 0
    while any(re.search(rf"\b{re.escape(candidate)}\b", text) for _, text in texts):
        suffix += 1
        candidate = f"{requested}__{digest}_{suffix}"
    return candidate
...
def expand_flist_path(raw: str, base: Path, project_root: Path) -> Path:
    expanded = os.path.expandvars(os.path.expanduser(raw))
    path = Path(expanded)
    if path.is_absolute():
        return path.resolve()
...
def parse_flist(flist: Path, project_root: Path, seen: set[Path] | None = None) -> FlistParseResult:
    ...
    for raw_line in flist.read_text(errors="ignore").splitlines():
        ...
        if line.startswith("-f "):
            nested = expand_flist_path(line[3:].strip(), base, project_root)
            ...
            continue
        if line.startswith("-F "):
            nested = expand_flist_path(line[3:].strip(), project_root, project_root)
            nested_result = parse_flist(nested, project_root, seen)
            result.files.extend(nested_result.files)
            result.lines.extend(nested_result.lines)
            result.incdirs.update(nested_result.incdirs)
        ...
        if line.startswith("+incdir+"):
            for item in line[len("+incdir+") :].split("+"):
                path = expand_flist_path(item, base, project_root)
        ...
        if line.startswith("-I"):
            ...
            continue
        first = line.split()[0]
        path = expand_flist_path(first, base, project_root)
...
def map_flist_line(raw_line: str, flist_base: Path, project_root: Path, out_dir: Path) -> str:
    ...
    if line.startswith("+incdir+"):
        ...
        mapped = []
        for item in dirs:
            path = expand_flist_path(item, flist_base, project_root)
            try:
    ...
    first = line.split()[0]
    path = expand_flist_path(first, flist_base, project_root)
...
def instrument_project(
    ...
    *,
    ...
    coverage_epoch_width: int | None = None,
    coverage_epoch_port: str = "coverage_epoch_i",
    force: bool = False,
) -> dict:
    ...
    coverage_port = _resolve_instrumentation_name(coverage_port, all_files, project_root)
    signal_prefix = _resolve_instrumentation_name(signal_prefix, all_files, project_root)
    next_index = 0
    file_plans: list[FilePlan] = []
    ...
    if resolved_settings.hierarchy.propagate_to_top:
        if coverage_epoch_width is not None and coverage_epoch_width <= 1:
            raise SystemExit("coverage epoch width must exceed one bit")
        module_map, top_modules, active_modules, module_coverage, hierarchy_skipped = add_hierarchy_insertions(
            file_plans, top_module, coverage_port,
            coverage_epoch_port=coverage_epoch_port if coverage_epoch_width is not None else None,
            coverage_epoch_width=coverage_epoch_width or 16,
        )
        required = set(required_modules) if required_modules is not None else set(top_modules)
        optional = set(optional_modules) if optional_modules is not None else active_modules - required
        overlap = required & optional
        if overlap:
            raise SystemExit("coverage modules cannot be both required and optional: " + ", ".join(sorted(overlap)))
        uncovered = active_modules - required - optional
        if uncovered:
            raise SystemExit(
                "coverage requirement missing for active module(s): " + ", ".join(sorted(uncovered))
            )
        coverage_abi = build_coverage_abi(
            module_map, top_modules, active_modules, coverage_port,
            required_modules=required, optional_modules=optional,
        )
    else:
    ...
    manifest = {
        ...
        "coverage_point_count": coverage_abi["width"] if coverage_abi is not None else len(points),
        ...
        "coverage_abi_hash": coverage_abi["manifest_digest"] if coverage_abi is not None else "",
        "coverage_inclusion_mask": (
            [bool(point["included"]) for point in coverage_abi["points"]]
            if coverage_abi is not None else []
        ),
        "coverage_required_point_count": (
            sum(1 for point in coverage_abi["points"] if point["requirement"] == "required")
            if coverage_abi is not None else 0
        ),
        "coverage_optional_point_count": (
            sum(1 for point in coverage_abi["points"] if point["requirement"] == "optional")
            if coverage_abi is not None else 0
        ),
        "coverage_skipped_point_count": (
            sum(1 for point in coverage_abi["points"] if not point["included"])
            if coverage_abi is not None else 0
        ),
    }
